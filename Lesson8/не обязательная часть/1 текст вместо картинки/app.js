...
function handleClick(clickedButtonEvent) {
    ...
    const card = {
        wrap: document.querySelector('.product'), // здесь элемент с классом .product
        img: document.querySelector('.product img'), // здесь картинка внутри .product
        productName: document.querySelector('.productName'), // здесь .productName, который внутри .product
        button: document.querySelector('button'), // здесь button, который внутри .product
    };

    // 4. получаем текст на кнопке, которая внутри .product
    const textInnerButton = this.innerText;

    if (textInnerButton === "Подробнее") { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (textInnerButton === "Отмена") { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function hideMoreText(card) {
    ...
    let descRemove = card.wrap.querySelector(".desc");
    descRemove.parentNode.removeChild(descRemove);
